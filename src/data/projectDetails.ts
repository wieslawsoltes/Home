...
const overrides: Record<string, Partial<ProjectDetail>> = {
  ...
  'dxf-parser': {
    introduction: 'DxfParser treats a DXF drawing as both structured data and visible geometry. It parses group-code pairs into a stable hierarchical tree, preserves malformed-container diagnostics, compares documents, builds a render scene, and supports inspection, editing, and round-trip export in a browser-native workbench.',
    audience: ['CAD and DXF implementers validating file structure', 'Engineering teams comparing or diagnosing drawings', 'Developers needing a browser-native DXF inspection and rendering reference'],
    architecture: [{ label: 'Parse', detail: 'Group-code and value pairs become typed tags, entities, containers, handles, and a stable hierarchical tree.' }, { label: 'Inspect', detail: 'Virtualized tree grids, statistics, handle navigation, diagnostics, filters, and document comparison expose the drawing structure.' }, { label: 'Render', detail: 'A document builder and retained scene graph feed Canvas and WebGL-oriented surfaces with text, layers, styles, and overlays.' }, { label: 'Edit + export', detail: 'The editor adapter mutates scene state and serializes the drawing back into DXF text.' }],
    compatibility: [{ label: 'Runtime', value: 'Modern browser / JavaScript', state: 'ready' }, { label: 'DXF 2018', value: 'Broad, actively tracked coverage', state: 'partial' }, { label: 'Canvas / WebGL', value: 'Rendering paths', state: 'ready' }, { label: 'NuGet', value: 'Not applicable', state: 'planned' }],
    proof: [{ value: 'O(n)', label: 'iterative tag and tree parsing' }, { value: '2 surfaces', label: 'Canvas and WebGL rendering' }, { value: '7 baselines', label: 'drawing regression fixtures' }],
    media: [{ src: raw('DxfParser', 'main', 'tests/outputs/complex-model.frame.svg'), alt: 'DxfParser rendered complex DXF model baseline', caption: 'A deterministic frame generated from the complex DXF regression fixture.' }, { src: raw('DxfParser', 'main', 'tests/outputs/advanced-geometry.frame.svg'), alt: 'DxfParser advanced geometry rendering baseline', caption: 'Advanced geometry exercises the parser, scene builder, and rendering surface together.' }],
    links: [{ label: 'Parser guide', href: repo('DxfParser', '/blob/main/docs/dxf-parser.md') }, { label: 'Rendering plan', href: repo('DxfParser', '/blob/main/docs/dxf-rendering-plan.md') }, { label: 'Test strategy', href: repo('DxfParser', '/blob/main/TESTING_STRATEGY.md') }],
    ...
    related: ['procad', 'core2d', 'svg-skia']
  },
  proedit: {
  ...
  htmlml: {
    introduction: 'HtmlML maps familiar HTML-shaped elements onto native Avalonia controls. Its companion JavaScript.Avalonia runtime adds a Jint engine, DOM traversal and mutation, routed-event bridging, modules, timers, animation frames, TypeScript transpilation, and Canvas APIs without hosting a browser engine.',
    audience: ['Avalonia teams bringing web markup concepts into native applications', 'Tool authors embedding scriptable native UI surfaces', 'Developers building hybrid C#/JavaScript or TypeScript workflows without a WebView'],
    architecture: [{ label: 'Markup', detail: 'HTML-like XAML tags map to concrete Avalonia controls and document structure.' }, { label: 'Style', detail: 'Classes, inline declarations, stylesheets, and converters apply CSS-inspired values through Avalonia properties.' }, { label: 'DOM + events', detail: 'JavaScript.Avalonia exposes visual-tree queries, mutation, event propagation, timers, modules, and native object access.' }, { label: 'Canvas', detail: '2D and WebGL-oriented contexts record drawing commands against Avalonia rendering surfaces.' }],
    compatibility: [{ label: 'Avalonia', value: 'Native desktop targets', state: 'ready' }, { label: 'JavaScript', value: 'Embedded Jint runtime', state: 'ready' }, { label: 'TypeScript', value: 'Embedded transpilation', state: 'ready' }, { label: 'NuGet', value: 'Publication planned', state: 'planned' }],
    proof: [{ value: '30+ tags', label: 'HTML-shaped element set' }, { value: 'DOM-style', label: 'queries, mutation, and events' }, { value: '2D + WebGL', label: 'Canvas directions' }],
    media: [{ src: 'https://opengraph.githubassets.com/portfolio-htmlml/wieslawsoltes/HtmlML', alt: 'HtmlML GitHub repository preview', caption: 'The repository combines the markup system, JavaScript runtime, website sample, native host sample, and interactive playground.' }],
    links: [{ label: 'HtmlML guide', href: repo('HtmlML', '/blob/main/src/HtmlML/README.md') }, { label: 'JavaScript.Avalonia guide', href: repo('HtmlML', '/blob/main/src/JavaScript.Avalonia/README.md') }, { label: 'Samples', href: repo('HtmlML', '/tree/main/samples') }],
    ...
    related: ['xaml-playground', 'nativewebview', 'nxui']
  },
  cdp: {
    introduction: 'CDP is a complete native UI testing and diagnostics platform. Test Studio records interactions, authors synchronized YAML and visual flows, infers assertions, runs step-by-step, captures screenshots and telemetry, and produces reports. The same protocol surface supports headless CI, Playwright, Puppeteer, Selenium, Appium, CLI and agent workflows.',
    audience: ['Desktop teams building repeatable end-to-end and regression suites', 'QA engineers authoring visual and YAML-native test flows', 'Framework, tooling, and agent authors needing inspectable native UI'],
    architecture: [{ label: 'Author', detail: 'Test Studio, recorder, visual node editor, YAML IntelliSense, selectors, environments, and reusable flows define intent.' }, { label: 'Run', detail: 'Desktop, CLI, headless, Playwright, Selenium, Appium, and OS automation drivers execute the same UI operations.' }, { label: 'Observe', detail: 'Screenshots, assertions, network events, CPU, memory, FPS, step timing, and reports capture evidence.' }, { label: 'Protocol', detail: 'CDP domains and Avalonia, WPF, WinUI, Uno, or OS adapters translate actions into native framework behavior.' }],
    compatibility: [{ label: 'Test Studio', value: 'Visual + YAML flows', state: 'ready' }, { label: 'Headless CI', value: 'Avalonia.Headless', state: 'ready' }, { label: 'Playwright / Selenium / Appium', value: 'Drivers', state: 'ready' }, { label: 'Avalonia / WPF / WinUI / Uno', value: 'Framework adapters', state: 'partial' }],
    proof: [{ value: '50+ commands', label: 'flow action catalog' }, { value: '4 test lanes', label: 'Studio, CLI, headless, external drivers' }, { value: 'Per step', label: 'screenshots and telemetry' }],
