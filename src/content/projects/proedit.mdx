...
story:
  ...
  title: A document is a system, not a text box.
  intro: ProEdit starts with a portable document model, then composes layout, rendering, editing, formats, reporting, printing, collaboration, and automation around it. Product shells share the engine without becoming the engine.
accent: "#ee8fff"
featured: true
tier: Flagship
status: Active
...
<StoryFlow
  label="From model to workflow"
  steps={[
    {
      signal: 'Authoring',
      label: 'Edit structured content',
      detail: 'Selections, commands, formatting, tables, lists, fields, revisions, and proofing operate on one model.',
      href: './document-editor/'
    },
    {
      signal: 'Exchange',
      label: 'Cross format boundaries',
      detail: 'DOCX, RTF, HTML, Markdown, PDF, PS, and XPS connect through focused import and export packages.',
      href: './document-formats/'
    },
    {
      signal: 'Output',
      label: 'Compose reports',
      detail: 'Reporting, pagination, print preview, and platform adapters reuse the same layout and rendering core.',
      href: './reporting/'
    },
    {
      signal: 'Shared state',
      label: 'Coordinate change',
      detail: 'Operations, synchronization, persistence, snapshots, and automation turn a document into a workflow.',
      href: './collaboration/'
    }
  ]}
/>

<StoryDecision label="Composition rule" title="Adopt the smallest complete slice.">
  A viewer should not need collaboration packages, and an automation service should not need a desktop shell. ProEdit keeps capabilities package-oriented so teams can **compose a coherent product without importing an unrelated platform surface**.
</StoryDecision>
